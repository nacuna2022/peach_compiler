...
struct token {
        int type; /* see TOKEN_TYPE_* enum above */
        int flags;
        struct pos pos;
        union {
                char cval;
        ...
        /* true if there is a whitespace between 
         * the token and the next token
         * i.e. '* a' for operator token * would mean whitespace would be 
         * set for token 'a' */
        bool whitespace;
...
struct lex_process {
        ...
        /* counts the number of brackets at the moment. e.g. ((50)) => 2 */
        int current_expression_count;
        struct buffer *parenthesis_buffer;
        struct lex_process_functions *function;

        /* this will be private date that the lexer does not understand 
